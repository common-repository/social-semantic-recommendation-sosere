...
jQuery('.admininput').on('click', '#default_thumb_button', function( event ){
	// prevent default
...
/*
* recommendation box title language select options presetting
*/
jQuery( '.sosere_rbt_language_select' ).each( function( lang ) {
	
	var sibEl = jQuery( this ).siblings( 'input' ).attr( 'name' ).split( '[' ).pop().slice( 0, -1 );
	var selected_lang = jQuery( this ).val();
	
	// set current selected language option value
	if ( sibEl != selected_lang ) {
		jQuery( this ).val( sibEl ); 
	}	
});
jQuery( '.sosere_rbt_language_select' ).change( function( lang ) {
	var selected_lang = jQuery( this ).val();
	jQuery( this ).siblings( 'input' ).attr( 'name', 'plugin_sosere[recommendation_box_title][' + selected_lang + ']' );
});
